/*
 * GRCNNHSWordCharClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_GRCNNHSWordCharClassifier_H_
#define SRC_GRCNNHSWordCharClassifier_H_
...
class GRCNNHSWordCharClassifier {
public:
  GRCNNHSWordCharClassifier() {
    _dropOut = 0.5;
  }
  ~GRCNNHSWordCharClassifier() {
...
public:
  ...
  GRNN<xpu> _char_rnn_left;
  GRNN<xpu> _char_rnn_right;
  UniLayer<xpu> _hchar_cnn_project;
  GRNN<xpu> _hchar_rnn_left;
  GRNN<xpu> _hchar_rnn_right;
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, const NRMat<dtype>& wordhEmb, const NRMat<dtype>& charEmb, const NRMat<dtype>& hcharEmb, int wordcontext,
    ...
    _hchars.initial(charEmb);
    _hchar_rnn_left.initial(_charHiddenSize, _char_cnn_iSize, true, 30);
    _hchar_rnn_right.initial(_charHiddenSize, _char_cnn_iSize, false, 70);
    _hchar_cnn_project.initial(_charHiddenSize, 2 * _charHiddenSize, true, 40, 0);
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      vector<vector<Tensor<xpu, 3, dtype> > > char_rnn_hidden_left_update, char_rnn_hidden_left_reset;
      vector<vector<Tensor<xpu, 3, dtype> > > char_rnn_hidden_left_afterreset, char_rnn_hidden_left_current;
      vector<vector<Tensor<xpu, 3, dtype> > > char_rnn_hidden_left, char_rnn_hidden_leftLoss;
      vector<vector<Tensor<xpu, 3, dtype> > > char_rnn_hidden_right_update, char_rnn_hidden_right_reset;
      vector<vector<Tensor<xpu, 3, dtype> > > char_rnn_hidden_right_afterreset, char_rnn_hidden_right_current;
      ...
      char_rnn_hidden_left_update.resize(seq_size);
      char_rnn_hidden_left_reset.resize(seq_size);
      char_rnn_hidden_left.resize(seq_size);
      char_rnn_hidden_leftLoss.resize(seq_size);
      char_rnn_hidden_left_afterreset.resize(seq_size);
      char_rnn_hidden_left_current.resize(seq_size);

      char_rnn_hidden_right_update.resize(seq_size);
      char_rnn_hidden_right_reset.resize(seq_size);
      char_rnn_hidden_right.resize(seq_size);
      char_rnn_hidden_rightLoss.resize(seq_size);
      char_rnn_hidden_right_afterreset.resize(seq_size);
      char_rnn_hidden_right_current.resize(seq_size);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        char_rnn_hidden_left_update[idx].resize(word_num);
        char_rnn_hidden_left_reset[idx].resize(word_num);
        char_rnn_hidden_left[idx].resize(word_num);
        char_rnn_hidden_leftLoss[idx].resize(word_num);
        char_rnn_hidden_left_afterreset[idx].resize(word_num);
        char_rnn_hidden_left_current[idx].resize(word_num);
        char_rnn_hidden_right_update[idx].resize(word_num);
        char_rnn_hidden_right_reset[idx].resize(word_num);
        char_rnn_hidden_right[idx].resize(word_num);
        char_rnn_hidden_rightLoss[idx].resize(word_num);
        char_rnn_hidden_right_afterreset[idx].resize(word_num);
        char_rnn_hidden_right_current[idx].resize(word_num);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          ...
          char_rnn_hidden_left_update[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_left_reset[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_left[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_leftLoss[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_left_afterreset[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_left_current[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);

          char_rnn_hidden_right_update[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_right_reset[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_right[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_rightLoss[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_right_afterreset[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
          char_rnn_hidden_right_current[idx][idy] = NewTensor<xpu>(Shape3(char_num, 1, _charHiddenSize), d_zero);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          if (idx == seq_size - 1) {
            _char_rnn_left.ComputeForwardScore(char_input[idx][idy], char_rnn_hidden_left_reset[idx][idy], char_rnn_hidden_left_afterreset[idx][idy],
                char_rnn_hidden_left_update[idx][idy], char_rnn_hidden_left_current[idx][idy], char_rnn_hidden_left[idx][idy]);
            _char_rnn_right.ComputeForwardScore(char_input[idx][idy], char_rnn_hidden_right_reset[idx][idy], char_rnn_hidden_right_afterreset[idx][idy],
                char_rnn_hidden_right_update[idx][idy], char_rnn_hidden_right_current[idx][idy], char_rnn_hidden_right[idx][idy]);
          } else {
            _hchar_rnn_left.ComputeForwardScore(char_input[idx][idy], char_rnn_hidden_left_reset[idx][idy], char_rnn_hidden_left_afterreset[idx][idy],
                char_rnn_hidden_left_update[idx][idy], char_rnn_hidden_left_current[idx][idy], char_rnn_hidden_left[idx][idy]);
            _hchar_rnn_right.ComputeForwardScore(char_input[idx][idy], char_rnn_hidden_right_reset[idx][idy], char_rnn_hidden_right_afterreset[idx][idy],
                char_rnn_hidden_right_update[idx][idy], char_rnn_hidden_right_current[idx][idy], char_rnn_hidden_right[idx][idy]);
          }
        }
        ...
        if (idx == seq_size - 1) {
          _rnn_left.ComputeForwardScore(input[idx], rnn_hidden_left_reset, rnn_hidden_left_afterreset, rnn_hidden_left_update, rnn_hidden_left_current,
              rnn_hidden_left);
          _rnn_right.ComputeForwardScore(input[idx], rnn_hidden_right_reset, rnn_hidden_right_afterreset, rnn_hidden_right_update, rnn_hidden_right_current,
              rnn_hidden_right);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        if (idx == seq_size - 1) {
          ...
          _rnn_left.ComputeBackwardLoss(input[idx], rnn_hidden_left_reset, rnn_hidden_left_afterreset, rnn_hidden_left_update, rnn_hidden_left_current,
              rnn_hidden_left, rnn_hidden_leftLoss, inputLoss[idx]);
          _rnn_right.ComputeBackwardLoss(input[idx], rnn_hidden_right_reset, rnn_hidden_right_afterreset, rnn_hidden_right_update, rnn_hidden_right_current,
              rnn_hidden_right, rnn_hidden_rightLoss, inputLoss[idx]);
        } else {
          for (int idy = 0; idy < word_num; idy++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          if (idx == seq_size - 1) {
            _char_rnn_left.ComputeBackwardLoss(char_input[idx][idy], char_rnn_hidden_left_reset[idx][idy], char_rnn_hidden_left_afterreset[idx][idy],
                char_rnn_hidden_left_update[idx][idy], char_rnn_hidden_left_current[idx][idy], char_rnn_hidden_left[idx][idy],
                char_rnn_hidden_leftLoss[idx][idy], char_inputLoss[idx][idy]);
            _char_rnn_right.ComputeBackwardLoss(char_input[idx][idy], char_rnn_hidden_right_reset[idx][idy], char_rnn_hidden_right_afterreset[idx][idy],
                char_rnn_hidden_right_update[idx][idy], char_rnn_hidden_right_current[idx][idy], char_rnn_hidden_right[idx][idy],
                char_rnn_hidden_rightLoss[idx][idy], char_inputLoss[idx][idy]);
          } else {
            _hchar_rnn_left.ComputeBackwardLoss(char_input[idx][idy], char_rnn_hidden_left_reset[idx][idy], char_rnn_hidden_left_afterreset[idx][idy],
                char_rnn_hidden_left_update[idx][idy], char_rnn_hidden_left_current[idx][idy], char_rnn_hidden_left[idx][idy],
                char_rnn_hidden_leftLoss[idx][idy], char_inputLoss[idx][idy]);
            _hchar_rnn_right.ComputeBackwardLoss(char_input[idx][idy], char_rnn_hidden_right_reset[idx][idy], char_rnn_hidden_right_afterreset[idx][idy],
                char_rnn_hidden_right_update[idx][idy], char_rnn_hidden_right_current[idx][idy], char_rnn_hidden_right[idx][idy],
                char_rnn_hidden_rightLoss[idx][idy], char_inputLoss[idx][idy]);
          }
        }
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          ...
          FreeSpace(&(char_rnn_hidden_left_update[idx][idy]));
          FreeSpace(&(char_rnn_hidden_left_reset[idx][idy]));
          FreeSpace(&(char_rnn_hidden_left[idx][idy]));
          FreeSpace(&(char_rnn_hidden_leftLoss[idx][idy]));
          FreeSpace(&(char_rnn_hidden_left_afterreset[idx][idy]));
          FreeSpace(&(char_rnn_hidden_left_current[idx][idy]));
          FreeSpace(&(char_rnn_hidden_right_update[idx][idy]));
          FreeSpace(&(char_rnn_hidden_right_reset[idx][idy]));
          FreeSpace(&(char_rnn_hidden_right[idx][idy]));
          FreeSpace(&(char_rnn_hidden_rightLoss[idx][idy]));
          FreeSpace(&(char_rnn_hidden_right_afterreset[idx][idy]));
          FreeSpace(&(char_rnn_hidden_right_current[idx][idy]));
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    checkgrad(this, examples, _char_rnn_left._rnn_update._WL, _char_rnn_left._rnn_update._gradWL, "_char_rnn_left._rnn_update._WL", iter);
    checkgrad(this, examples, _char_rnn_left._rnn_update._WR, _char_rnn_left._rnn_update._gradWR, "_char_rnn_left._rnn_update._WR", iter);
    checkgrad(this, examples, _char_rnn_left._rnn_update._b, _char_rnn_left._rnn_update._gradb, "_char_rnn_left._rnn_update._b", iter);

    checkgrad(this, examples, _char_rnn_right._rnn_update._WL, _char_rnn_right._rnn_update._gradWL, "_char_rnn_right._rnn_update._WL", iter);
    checkgrad(this, examples, _char_rnn_right._rnn_update._WR, _char_rnn_right._rnn_update._gradWR, "_char_rnn_right._rnn_update._WR", iter);
    checkgrad(this, examples, _char_rnn_right._rnn_update._b, _char_rnn_right._rnn_update._gradb, "_char_rnn_right._rnn_update._b", iter);

    checkgrad(this, examples, _char_rnn_left._rnn_reset._WL, _char_rnn_left._rnn_reset._gradWL, "_char_rnn_left._rnn_reset._WL", iter);
    checkgrad(this, examples, _char_rnn_left._rnn_reset._WR, _char_rnn_left._rnn_reset._gradWR, "_char_rnn_left._rnn_reset._WR", iter);
    checkgrad(this, examples, _char_rnn_left._rnn_reset._b, _char_rnn_left._rnn_reset._gradb, "_char_rnn_left._rnn_reset._b", iter);

    checkgrad(this, examples, _char_rnn_right._rnn_reset._WL, _char_rnn_right._rnn_reset._gradWL, "_char_rnn_right._rnn_reset._WL", iter);
    checkgrad(this, examples, _char_rnn_right._rnn_reset._WR, _char_rnn_right._rnn_reset._gradWR, "_char_rnn_right._rnn_reset._WR", iter);
    checkgrad(this, examples, _char_rnn_right._rnn_reset._b, _char_rnn_right._rnn_reset._gradb, "_char_rnn_right._rnn_reset._b", iter);
    ...
    checkgrad(this, examples, _hchar_rnn_left._rnn_update._WL, _hchar_rnn_left._rnn_update._gradWL, "_hchar_rnn_left._rnn_update._WL", iter);
    checkgrad(this, examples, _hchar_rnn_left._rnn_update._WR, _hchar_rnn_left._rnn_update._gradWR, "_hchar_rnn_left._rnn_update._WR", iter);
    checkgrad(this, examples, _hchar_rnn_left._rnn_update._b, _hchar_rnn_left._rnn_update._gradb, "_hchar_rnn_left._rnn_update._b", iter);

    checkgrad(this, examples, _hchar_rnn_right._rnn_update._WL, _hchar_rnn_right._rnn_update._gradWL, "_hchar_rnn_right._rnn_update._WL", iter);
    checkgrad(this, examples, _hchar_rnn_right._rnn_update._WR, _hchar_rnn_right._rnn_update._gradWR, "_hchar_rnn_right._rnn_update._WR", iter);
    checkgrad(this, examples, _hchar_rnn_right._rnn_update._b, _hchar_rnn_right._rnn_update._gradb, "_hchar_rnn_right._rnn_update._b", iter);

    checkgrad(this, examples, _hchar_rnn_left._rnn_reset._WL, _hchar_rnn_left._rnn_reset._gradWL, "_hchar_rnn_left._rnn_reset._WL", iter);
    checkgrad(this, examples, _hchar_rnn_left._rnn_reset._WR, _hchar_rnn_left._rnn_reset._gradWR, "_hchar_rnn_left._rnn_reset._WR", iter);
    checkgrad(this, examples, _hchar_rnn_left._rnn_reset._b, _hchar_rnn_left._rnn_reset._gradb, "_hchar_rnn_left._rnn_reset._b", iter);

    checkgrad(this, examples, _hchar_rnn_right._rnn_reset._WL, _hchar_rnn_right._rnn_reset._gradWL, "_hchar_rnn_right._rnn_reset._WL", iter);
    checkgrad(this, examples, _hchar_rnn_right._rnn_reset._WR, _hchar_rnn_right._rnn_reset._gradWR, "_hchar_rnn_right._rnn_reset._WR", iter);
    checkgrad(this, examples, _hchar_rnn_right._rnn_reset._b, _hchar_rnn_right._rnn_reset._gradb, "_hchar_rnn_right._rnn_reset._b", iter);
...
#endif /* SRC_GRCNNHSWordCharClassifier_H_ */
